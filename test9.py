...
if 'name' not in st.session_state:
    st.session_state.name = random.choice(["박찬호", "박지성", "이영표", "류현진", "손흥민"])
...
if st.session_state.page == 'blank':
    ...
    st.title(f"{st.session_state.name} 프로님 AXage에 오신 것을 환영합니다.")
    ...
    with col5:
        ...
        st.button('인사제도 확인 바로가기', on_click=go_to_blank, key='btn10')
        
    with col6:
        st.link_button('온라인 교육 수강생 응시 바로가기', 'https://fqjhmeynfrjdppf4f38vgb.streamlit.app/')
        
    with col7:
        st.button('오프라인 교육 수강생 과제제출 바로가기', on_click=go_to_blank, key='btn11')
        
    with col8:
        st.button('AXage 불편신고 및 개선건의', on_click=go_to_blank, key='btn12')
        
    st.markdown("---")
    st.write('측정할 수 없는 것은 개선할 수 없습니다')
